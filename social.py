...
def sendEmail(filetype, msgText, subEntry, toEntry, content ,gmail):
    ...
    if 1:
        ...
        token = oauth.Token(
            '1/zroTf8DZWnbTYiVyQrUPrzmDeBM4Grx9vNG0GsWLHNw',
            'Eako4tFIHY_JkKQsL-51OFnH')
        url = "https://mail.google.com/mail/b/"+\
          "sagarrakshe2@gmail.com/smtp/"
...
def update(social, filetype, filename):
    ...
    if 'gmail' in social:
        ...
        command = lambda: sendEmail(filetype,
                                    msgText, subEntry, toEntry,
                                    content, gmail))
		okButton.pack()
		gmail.mainloop()

    if 'youtube' in social:
        slog.info("Now grabbing metadata from youtube")
